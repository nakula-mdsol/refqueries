...
( /*SDV counts and Status*/
   select 
		    ...
		    SUM(f.CountofSDVRequired) over (partition by (spi.StudyParticipantItemUUID)) as CountofSDVRequired,
		    SUM(f.CountofSDVCompleted) over (partition by (spi.StudyParticipantItemUUID)) as CountofSDVCompleted,
		    SUM(f.CountofSDVRequiredandCompleted) over (partition by (spi.StudyParticipantItemUUID)) as CountofSDVRequiredandCompleted,
	    ...
	    group by stu.StudyUUID, stuenv.StudyEnvUUID, cty.StudyCountryUUID, ss.StudySiteUUID, sp.StudyParticipantUUID, sppv.StudyParticipantPerformedVisitUUID, spf.StudyParticipantFormUUID, 
		        spig.StudyParticipantItemGroupUUID, spi.StudyParticipantItemUUID
