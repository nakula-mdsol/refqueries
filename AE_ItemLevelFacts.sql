...
select      stu.STU_uuid,
            ...
            max(f.sys_dt_time),
MAX(f.CNT_OF_SDV_REQ) as CountofSDVRequired,
MAX(f.CNT_OF_SDV_COMPLETED) as CountofSDVCompleted,
MAX(f.CNT_OF_SDV_REQ_AND_COMPLETED) as CountofSDVRequiredandCompleted,
Case 
when MAX(f.CNT_OF_SDV_REQ) >0 and MAX(f.CNT_OF_SDV_REQ_AND_COMPLETED) = MAX(f.CNT_OF_SDV_REQ) then 'SDV Complete' 
when MAX(f.CNT_OF_SDV_REQ) >0 and MAX(f.CNT_OF_SDV_REQ_AND_COMPLETED) < SUM(f.CNT_OF_SDV_REQ) then 'SDV Partially Complete'
when MAX(f.CNT_OF_SDV_REQ) =0 and MAX(f.CNT_OF_SDV_REQ_AND_COMPLETED) >=0 then 'SDV Not required'
end as SDVStatus  
from FACT_STU_PNT_ITM_AGG_SDVSTATUS_RAVE f
		...
		where (f.STU_PNT_ITM_KEY,f.BUSINESS_DT_TIME,f.SYS_DT_TIME) IN 
                         (select stu_pnt_itm_key,row_eff_at,load_dt
                                   from DIM_STU_PNT_ITM 
                                   where (STU_PNT_ITM_UUID,row_eff_at,load_dt) IN
                                 (select distinct stu_pnt_itm_uuid, 
                                   max(row_eff_at) over (partition by stu_pnt_itm_uuid,row_eff_at), 
                                   max(load_dt) over (partition by stu_pnt_itm_uuid,row_eff_at) 
                                   from DIM_STU_PNT_ITM  
                                   where stu_pnt_itm_uuid IN
                                        ( select stu_pnt_itm_uuid
                                          from DIM_STU_PNT_ITM spi 
                                          where STU_UUID IN (TO_BINARY('11EDD461333B3E0998110E1498653597'))
                                          and row_eff_at <= '2024-01-21' 
                                         )))
 and stu.STU_KEY IN ( select STU_KEY
                           from DIM_STU stu
                           where (stu_uuid, stu.row_eff_at,load_dt) IN
                                (select stu_uuid, row_eff_at, max(load_dt) over (partition by stu_uuid, row_eff_at)
                                     from DIM_STU stu
                                     where stu.STU_UUID IN (TO_BINARY('11EDD461333B3E0998110E1498653597'))
                    ...
                    (select audit_key,row_eff_at,load_dt
                     ...
                     where (AUDIT_UUID,row_eff_at,load_dt) IN 
                            (select distinct spia.audit_uuid, spia.row_eff_at,
                                max(spia.load_dt) over (partition by audit_uuid,row_eff_at)  
                             from DIM_STU_PNT_ITM_AUDIT spia
                             where spia.STU_PNT_ITM_UUID IN 
                                     (select distinct spi.stu_pnt_itm_uuid 
                                      from DIM_STU_PNT_ITM spi
                                      where spi.STU_UUID IN (TO_BINARY('11EDD461333B3E0998110E1498653597'))
...
and f.STU_KEY IN ( select STU_KEY
                           from DIM_STU stu
                           where (stu_uuid, stu.row_eff_at,load_dt) IN
                                (select stu_uuid, row_eff_at, max(load_dt) over (partition by stu_uuid, row_eff_at)
                                     from DIM_STU stu
